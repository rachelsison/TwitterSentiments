...
def analyze_tweet_sentiment(tweet):
    ...
    score, applicable_words = 0, 0

    for elem in tweet_words(tweet):
        if has_sentiment(make_sentiment(get_word_sentiment(elem))):
            score += get_word_sentiment(elem)
            applicable_words += 1
    
    if applicable_words == 0:
        return average
    else:
        average = (score / applicable_words)
        return average
...
def find_centroid(polygon):
    ...
    (1, 2, 0)
    """
    A = (1/2) * sum((latitude(polygon[elem]) * longitude(polygon[elem+1])) - (latitude(polygon[elem+1]) * longitude(polygon[elem])) for elem in range(len(polygon)-1)) 
    
    if A == 0:
        A = int(A)
        Cx = latitude(polygon[0]) 
        Cy = longitude(polygon[1])
    else:
        Cx = (1/(6 * A)) * sum(((latitude(polygon[elem]) + latitude(polygon[elem+1])) * ((latitude(polygon[elem]) * longitude(polygon[elem+1])) - (latitude(polygon[elem+1]) * longitude(polygon[elem]))) for elem in range(len(polygon)-1)))
        Cy = (1/(6 * A)) * sum(((longitude(polygon[elem+1]) + longitude(polygon[elem])) * ((latitude(polygon[elem]) * longitude(polygon[elem+1])) - (latitude(polygon[elem+1]) * longitude(polygon[elem]))) for elem in range(len(polygon)-1)))
...
def find_center(polygons):
    ...
    if len(polygons) == 1:
        return find_centroid(polygons[0])
    else:
        for elem in range(len(polygons)):
            poly_cracker.append(find_centroid(polygons[elem]))


    for elem in poly_cracker:
        Cix += (elem[0] * elem[2])
        Ciy += (elem[1] * elem[2])
        Ai += elem[2]
