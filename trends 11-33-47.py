...
def analyze_tweet_sentiment(tweet):
    ...
    total, k = 0, 0
    for elem in extract_words(tweet['text']):
        if has_sentiment(make_sentiment(get_word_sentiment(elem))):
            total += get_word_sentiment(elem)
            k += 1
    if total == 0:
        return average
    else:
...
def find_centroid(polygon):
    ...
    n = len(polygon) - 1
    A = reduce(add, map(lambda x: latitude(polygon[x])*longitude(polygon[x+1])-latitude(polygon[x+1])*longitude(polygon[x]), range(n)))/2
    if A == 0:
        return latitude(polygon[0]), longitude(polygon[0]), int(A) 
    else:
        Cx = reduce(add, map(lambda x: (latitude(polygon[x])+latitude(polygon[x+1]))*(latitude(polygon[x])*longitude(polygon[x+1])-latitude(polygon[x+1])*longitude(polygon[x])), range(n)))/(6*A)
        Cy = reduce(add, map(lambda x: (longitude(polygon[x])+longitude(polygon[x+1]))*(latitude(polygon[x])*longitude(polygon[x+1])-latitude(polygon[x+1])*longitude(polygon[x])), range(n)))/(6*A)
...
def find_center(polygons):
    ...
    n = len(polygons) 
    if n == 0:
        return (find_centroid(polygons[0])[0], find_centroid(polygons[0])[1])
    A_tot = reduce(add, map(lambda x: find_centroid(polygons[x])[2], range(n)))
    Cx = (reduce(add, map(lambda x: (find_centroid(polygons[x])[2])*(find_centroid(polygons[x])[0]), range(n))))/ A_tot 
    Cy = (reduce(add, map(lambda x: (find_centroid(polygons[x])[2])*(find_centroid(polygons[x])[1]), range(n))))/ A_tot
...
def find_closest_state(tweet, state_centers):
    ...
    for s in state_centers:
        if i == 0:
            minn = geo_distance(state_centers.get(s), (tweet['latitude'], tweet['longitude']))
            min_state = s
            i += 1
        else:
            if geo_distance(state_centers.get(s), (tweet['latitude'], tweet['longitude'])) < minn:
                minn = geo_distance(state_centers.get(s), (tweet['latitude'], tweet['longitude']))
                min_state = s
    return min_state
...
def most_talkative_state(term):
    ...
    tweet_dic = group_tweets_by_state(tweets)
    m = 0
    state = None
    for k in tweet_dic:
        if len(tweet_dic[k]) > m:
            m = len(tweet_dic[k])
            state = k 
    return state


    max(len(group_tweets_by_state(tweets).get))
    ...
    for state in tweets_by_state:
        ...
        for tweet in tweets_by_state[state]:
            if analyze_tweet_sentiment(tweet) != None:
                K += analyze_tweet_sentiment(tweet)
                y += 1
        if y != 0:
            averaged_state_sentiments[state] = K/y
...
def group_tweets_by_hour(tweets):
    ...
    """
    for s in tweets:
        if s['time'].hour in tweets_by_hour:
            tweets_by_hour[s['time'].hour].append(s)
    else:
        tweets_by_hour[s['time'].hour] = [s]
